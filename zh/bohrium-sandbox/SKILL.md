...
```bash
# 必须装 prerelease，否则没有 sdbx 子命令
python3 -m pip install --pre --upgrade lbg

# 验证
python sdbx.py --help    # 应能看到 doctor / create / list / exec / files / terminal 等子命令
```
...
```bash
python sdbx.py doctor --json   # 检查鉴权 / SDK / 网关
```
...
```bash
python sdbx.py list --json                                   # 列出我的沙箱
python sdbx.py describe <sandbox_id> --with-processes --json # 元信息 + 进程
python sdbx.py ps <sandbox_id> --json                        # 进程列表
```
...
```bash
python sdbx.py template ls              # 列我创建过的模板
python sdbx.py template ls --json
python sdbx.py template ls -q           # 仅 name，方便管道喂给 create

# 创建模板（需要 image path + SKU name）
lbg image ls                      # 找 image path
python sdbx.py machine list             # 找 SKU
...
- 状态查询：`python sdbx.py ps <id>` 看 pid 是否还在；或 `files read` 看日志
...
python sdbx.py files read <id> /workspace/out/run.log  # 边跑边瞄
...
```bash
python sdbx.py terminal create <sandbox_id> --json              # 默认 timeout=0
...
python sdbx.py terminal kill   <sandbox_id> <pid> --json        # 只杀 pty，不杀沙箱
```
...
- **复用而非每次新建**：先 `python sdbx.py list` 看有没有空闲的，重复短任务最好挂在同一个沙箱上。
- **超过 30 分钟未用要 kill**：`python sdbx.py list` 会高亮警告；长期沙箱仍在计费。
...
- **sandbox** 调试镜像 → `lbg image ls` 找镜像；`python sdbx.py template create` 出新模板
